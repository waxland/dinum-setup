...
```bash
git clone https://github.com/waxland/dinum-setup.git
cd dinum-setup
```
...
5. **[Configuration Serveur Distant & VM](/01-onboarding/01-demarrage/configuration-serveur/01-guide-configuration-serveur)** : Configurer La Suite Docs sur une machine distante avec IP publique et consulter la **[Proposition de PR DINUM](/01-onboarding/01-demarrage/configuration-serveur/02-pr-support-serveurs-distants)**.
6. **[Configuration VS Code & Outils](/01-onboarding/01-demarrage/vscode)** : Installer les extensions recommandées (SQLTools 1-clic, GitLens, Ruff, ESLint) et le formatage automatique.
7. **[Workflow & Commandes Make](/01-onboarding/02-workflow-et-contribution/workflow)** : Découvrir toutes les cibles du `Makefile` et le cycle de vie de dev.
8. **[Tests, Linters & Qualité](/01-onboarding/02-workflow-et-contribution/tests-et-qualite)** : Exécuter les suites de tests unitaires, linters et tests E2E.
9. **[Guide de Dépannage & FAQ](/01-onboarding/03-support/troubleshooting)** : Diagnostiquer et résoudre les erreurs fréquentes (Docker, ports, BDD).
10. **[Architecture Globale](/02-architecture/index)** : Comprendre les flux, schémas Mermaid et choix techniques de l'écosystème.
11. **[Projets de La Suite](/03-projets/index)** : Explorer les fiches détaillées de chaque application.
12. **[Design System & DSFR](/04-design-system/index)** : Découvrir les composants UI, la palette institutionnelle et les règles d'accessibilité.
