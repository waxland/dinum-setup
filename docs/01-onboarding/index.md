...
👉 **[Consulter le répertoire complet des Services, URLs & Identifiants](urls-et-identifiants.md)** pour le détail exhaustif des bases de données PostgreSQL, consoles S3, webmails et clés secrètes.
...
1. **[Services, URLs & Identifiants](urls-et-identifiants.md)** : Répertoire exhaustif des URLs locales, ports et accès BDD/admin.
2. **[Configuration Git & Clés SSH](git-ssh.md)** : Initialiser votre profil Git et configurer vos accès SSH.
3. **[Configuration VS Code & Outils](vscode.md)** : Installer les extensions recommandées (GitLens, Git Graph, Ruff, ESLint) et le formatage automatique.
4. **[Workflow & Commandes Make](workflow.md)** : Découvrir toutes les cibles du `Makefile` et le cycle de vie de dev.
5. **[Tests, Linters & Qualité](tests-et-qualite.md)** : Exécuter les suites de tests unitaires, linters et tests E2E.
6. **[Guide de Dépannage & FAQ](troubleshooting.md)** : Diagnostiquer et résoudre les erreurs fréquentes (Docker, ports, BDD).
7. **[Architecture Globale](../02-architecture/index.md)** : Comprendre les flux, schémas Mermaid et choix techniques de l'écosystème.
8. **[Projets de La Suite](../03-projets/index.md)** : Explorer les fiches détaillées de chaque application.
9. **[Design System & DSFR](../04-design-system/index.md)** : Découvrir les composants UI, la palette institutionnelle et les règles d'accessibilité.
