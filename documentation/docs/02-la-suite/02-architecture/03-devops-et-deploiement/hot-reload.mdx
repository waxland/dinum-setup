...
- Les répertoires de code locaux (`./LaSuite/<projet>/...`) sont montés en **volumes Docker bind-mounts** dans les conteneurs.
...
- **Actif par défaut avec `make dev`** : Les fichiers Python sous `LaSuite/<projet>/src/backend/` sont montés dans `/app`.
...
- **Actif par défaut avec `make dev`** : Le code TypeScript sous `LaSuite/docs/src/frontend/servers/y-provider/` est surveillé et recompilé à chaud.
...
- **Option Locale Directe (Recommandée pour les développeurs frontend)** :
  ...
  make -C LaSuite/docs run-backend

  # 2. Démarrer le frontend en local sur votre machine
  make -C LaSuite/docs run-frontend-development
  ```
